...
{
    //Initialize i2c, set gpio to low level
    /* wakeup CH422G, enable I/O feature */
...
{
    // static uint8_t write_buf=0x0D;
    static uint8_t write_buf=0x0;
    // read first,and clear bit, set bit
    // ESP_LOGI(TAG,"write_buf before %d",write_buf);
    if(enable){
        write_buf |= 1 << num;
    ...
    // i2c_master_write_to_device(BSP_I2C0_NUM, 0x38, &write_buf, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
    // ESP_LOGI(TAG,"write_buf after %d",write_buf);
    return ESP_OK;
}
